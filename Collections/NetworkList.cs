...
namespace NetworkWrappers
{
    ...
    public class NetworkList<T> : NetworkClass, IList<T>, IReadOnlyList<T> where T : class
    {
        ...
        public NetworkList(IEnumerable<T> collection)
        {
        ...
        public override bool NetRead(NetRead read)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    throw new NotSupportedException("You can only network NetworkClass, Entity, string or INetIdentifiable. Use a struct with NetworkValueList<T> for other types.");
                }
            }
            return true;
        }
