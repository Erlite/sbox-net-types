...
namespace NetworkWrappers
{
    ...
    public class NetworkDictionaryCU<TKey, TValue> : NetworkClass, IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue>
        where TKey : class
        where TValue : unmanaged
    {
        ...
        #region Constructors
        public NetworkDictionaryCU()
        {
            _internalDictionary = new Dictionary<TKey, TValue>();
        }

        public NetworkDictionaryCU(int capacity)
        {
            _internalDictionary = new Dictionary<TKey, TValue>(capacity);
        }

        public NetworkDictionaryCU(IEqualityComparer<TKey>? comparer) : this(0, comparer)
        {
        }

        public NetworkDictionaryCU(int capacity, IEqualityComparer<TKey>? comparer)
        {
            _internalDictionary = new Dictionary<TKey, TValue>(capacity, comparer);
        }

        public NetworkDictionaryCU(IDictionary<TKey, TValue> dictionary) : this(dictionary, null)
        {
        }

        public NetworkDictionaryCU(IDictionary<TKey, TValue> dictionary, IEqualityComparer<TKey>? comparer)
            : this(dictionary?.Count ?? 0, comparer)
        {
            _internalDictionary = new Dictionary<TKey, TValue>(dictionary, comparer);
        }

        public NetworkDictionaryCU(IEnumerable<KeyValuePair<TKey, TValue>> collection) : this(collection, null)
        {
        }

        public NetworkDictionaryCU(IEnumerable<KeyValuePair<TKey, TValue>> collection, IEqualityComparer<TKey>? comparer) :
